...
export async function POST(request: NextRequest) {
  ...
  const upstreamResponse = await fetch(new URL("/api/v1/auth/login", getServerApiBaseUrl()), {
    method: "POST",
    cache: "no-store",
    headers,
    body,
  ...
  const response = NextResponse.json({ access_token: "session", token_type: "bearer" });
